...
Require `blueprint_generated: true` and `blueprint_approved: true`. Read every slide entry and image prompt before generation.
...
Inspect each candidate at useful size for count, aspect ratio, exact text and spelling, clipping, artifacts, continuity, prompt compliance, cultural constraints, and blueprint alignment. Regenerate obvious technical failures before presenting the set. Create `04_images/image_manifest.md` with one row per slide containing the image-prompt reference, initial file, current file, revision history, inspection status, and warnings. Set `images_generated` to true only when the complete set passes this technical inspection.
...
7. Inspect the revision, update the manifest's `current file`, and ask the professor to review it.
