...
Require `source_copy_created: true`. Work only from `00_source_original/`; never touch the professor's original dump.

## Organize

1. Inventory every file recursively with relative path, extension, byte size, and detected type where useful.
